...
public class TestGallery extends LockedTestCase {
	...
	public void test() {
		//g1 is empty
		it = g1.iterator();
		assertEquals(Tb(462356545), it.hasNext());
		// 0 = null
		// 1 = some non-null Painting object
		// -1 = IllegalArgumentException
		// -2 = NoSuchElementException
		...
		assertEquals(Ti(173242698), ex(() -> it.next()));
		assertEquals(Tb(430519694), g1.add(p1));
		assertEquals(Ti(542291396), g1.size());
		it = g1.iterator();
		assertEquals(Tb(1526862225), it.hasNext());
		assertEquals(Ti(1594826932), ex(() -> it.next()));
		assertEquals(Tb(1145153061), it.hasNext());
		it.remove();
	...
	private void testcont(Gallery g1) {
		...
		// -4 = ConcurrentModificationException
		// -5 = Some other exception, or other value
		assertEquals(Ti(1418375490), ex(() -> it.next()));
		g1.add(p3);
		it = g1.iterator();
		assertEquals(Tb(982051950), g1.add(p2));
		assertEquals(Ti(1172609459), ex(() -> p[0] = it.next()));
		it.remove();
		assertEquals(Ti(1282210448), ex(() -> p[1] = it.next()));
		g1.remove(p[0]);
		assertEquals(Ti(1381783689), ex(() -> p[2] = it.next()));
		g1.remove(p[1]);
		assertEquals(Ti(246256019), ex(() -> it.next()));
